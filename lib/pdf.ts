  ...
  for (const section of sections) {
    ...
    for (const item of items) {
      ...
      if (moveInCode) {
        ...
        if (isMoveOut) { doc.setFillColor(230, 240, 255) } else { doc.setFillColor(220, 245, 230) }
        doc.roundedRect(codeX, y + 1, 10, 4, 1, 1, 'F')
        doc.setFont('helvetica', 'bold')
        doc.setFontSize(7)
        if (isMoveOut) { doc.setTextColor(24, 95, 165) } else { doc.setTextColor(15, 110, 86) }
      ...
      if (isMoveOut && moveOutCode) {
        ...
        if (isDamaged) { doc.setFillColor(255, 220, 220) } else { doc.setFillColor(255, 245, 220) }
        doc.roundedRect(codeX, y + 1, 10, 4, 1, 1, 'F')
        doc.setFont('helvetica', 'bold')
        doc.setFontSize(7)
        if (isDamaged) { doc.setTextColor(180, 30, 30) } else { doc.setTextColor(180, 120, 20) }
